...
implements Comparable<State> {
    public abstract void prettyPrint(PrintStream var1);

    public abstract String toString();

    public abstract String encode();

    public static void prettyPrintStateSet(Collection<State> states, PrintStream out) {
        out.print("{");
    ...
    public static String encodeStateSet(Set<State> states) {
        ...
        if (p.hasNext()) {
            encoding = String.valueOf(encoding) + ((State)p.next()).encode();
        }
        while (p.hasNext()) {
            encoding = String.valueOf(encoding) + " " + ((State)p.next()).encode();
        }
